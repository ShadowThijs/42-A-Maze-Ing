        ...
        for raw_line in fh:
            ...
            if key == "WIDTH":
                try:
                    ...
                    raise ValueError(
                        f"WIDTH must be an integer, got {value!r}"
                    )
            elif key == "HEIGHT":
                try:
                    ...
                    raise ValueError(
                        f"HEIGHT must be an integer, got {value!r}"
                    )
            elif key == "ENTRY":
                parts = value.split(",")
                if len(parts) != 2:
                    raise ValueError(
                        f"ENTRY must be col,row (2 integers), got {value!r}"
                    )
                try:
                    config.Entry = [int(parts[0]), int(parts[1])]
                except ValueError:
                    raise ValueError(
                        f"ENTRY must be integers, got {value!r}"
                    )
            elif key == "EXIT":
                parts = value.split(",")
                if len(parts) != 2:
                    raise ValueError(
                        f"EXIT must be col,row (2 integers), got {value!r}"
                    )
                try:
                    config.Exit = [int(parts[0]), int(parts[1])]
                except ValueError:
                    raise ValueError(
                        f"EXIT must be integers, got {value!r}"
                    )
                ...
                if low not in ("true", "false"):
                    raise ValueError(
                        f"PERFECT must be 'True' or 'False', got {value!r}"
                    )
                config.Perfect = low == "true"
            elif key == "SEED":
...
def _enforce_no_3x3(
    ...
    """Break any 3x3 open zones by re-adding a wall.

    A 3x3 open zone is a 3x3 block of cells where all 12 internal walls
    are absent.  This iterates until no such zones remain.
    ...
    while changed:
        changed = False
        for oy in range(config.Height - 2):
            for ox in range(config.Width - 2):
                open_walls: list[tuple[str, int, int, int, int]] = []
                for dy in range(3):
                    for dx in range(2):
                        if not grid[ox + dx][oy + dy].walls["E"]:
                            open_walls.append(
                                ("E", ox + dx, oy + dy, ox + dx + 1, oy + dy)
                            )
                    if dy < 2:
                        for dx in range(3):
                            if not grid[ox + dx][oy + dy].walls["S"]:
                                open_walls.append(
                                    ("S", ox + dx, oy + dy,
                                     ox + dx, oy + dy + 1)
                                )
                if len(open_walls) == 12:
                    d, wx, wy, nx, ny = random.choice(open_walls)
                    grid[wx][wy].walls[d] = True
                    grid[nx][ny].walls[OPPOSITE[d]] = True
                    changed = True
